import { getAreaServices, getAreas, getGuests, getServices } from "./database.js";
...
export const areaList = () => {
    ...
    for (const area of areas) {
        areaHtml += `<li class="area" data-type="area" data-id="${area.id}" data-name="${area.trailName}"> ${area.section}<br>${area.trailName}<br>`
...
document.addEventListener(
    ...
    (clickEvent) => {
        ...
        if (itemClicked.dataset.type === "area") {
            ...
            window.alert(`${itemClicked.dataset.name} has ${counter} guests in the area`)
        }
    }
)
